...
def parse_args():
    ...
    parser.add_argument(
        "--router-path",
        type=str,
        default="experiments/mixture_of_experts/checkpoints/best_router.pth",
    )
    ...
    parser.add_argument(
        ...
        help="Optional calibrated temperatures",
    )

    return parser.parse_args()


def load_moe_ensemble_helper(
    expert_paths,
    checkpoint_dir,
    router_path,
    fe_idx,
    ...
    moe_model = MoEEnsemble(
        experts, router, feature_provider_idx=fe_idx, temperatures=temperatures
    ).to(device)
    moe_model.eval()
    return moe_model
...
def evaluate_moe():
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(test_loader):
            ...
            # We use the same temperature-scaled logits for a fair comparison
            expert_logits = []
            for i, expert in enumerate(moe_model.experts):
                logits, _ = expert(inputs)
                expert_logits.append(logits / moe_model.temps[i])
    ...
    # 6. Results Summary
    moe_acc = 100.0 * correct_moe / total
    mean_acc = 100.0 * correct_mean / total

    avg_weights = torch.cat(all_weights, dim=0).mean(dim=0).tolist()
    expert_names = [e.model_name.upper() for e in moe_model.experts]
