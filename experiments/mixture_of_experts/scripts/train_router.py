...
def parse_args():
    ...
    parser.add_argument("--save-dir", "--save_dir", type=str, default="experiments/mixture_of_experts/checkpoints", help="Where to save router")
    ...
    parser.add_argument("--fe-idx", type=int, default=3, help="Index of expert to provide features (default 3: ViT)")
    parser.add_argument("--fe-dim", type=int, default=768, help="Dimension of router input features (ViT: 768, ResNet34: 512, DenseNet121: 1024, EffNet: 1280)")
...
def train_router():
    ...
    for exp_path in args.experts:
        ...
        # Wrap the model for feature extraction
        wrapped = FeatureWrapper(model, name)
        wrapped.eval() # Experts are always in eval mode
    ...
    for epoch in range(args.epochs):
        ...
        print(f"   Epoch {epoch+1} Summary: Val Loss: {val_loss/len(val_loader):.4f} | Val Acc: {val_acc:.2f}%")
